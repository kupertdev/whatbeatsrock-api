...
class Game:
    def __init__(self, username: str = None, wait_for_response_time: float = 10.0, connect_timeout: float = 5.0, logging_level: int = logging.DEBUG):
    ...
    async def make_request(self, method: str = None, url: str = None, json_data: dict = None) -> dict:
        async with httpx.AsyncClient(timeout=httpx.Timeout(self.wait_for_response_time, connect=self.connect_timeout)) as client:
    ...
    async def play_game(self):
        ...
        if not self.username:
            ...
            keys = ["previous_guess", "judgingCriteria", "judgingCriteriaLoss", "room_id"]
            ...
            values = [game_info[start_key]["startWord"], game_info[start_key]["judgingCriteria"], game_info[start_key]["judgingCriteriaLoss"]]
        ...
        while True:
            ...
            result: dict = await self.game_request(previous_guess, guessed_word, room_id)
            ...
            if result.get("guess_wins", False) is False:
                ...
                print(f"Sorry dear, you lose! {prev_word} {loss_phrase} {guessed_word}!\n\n{result.get('reason', '')}")
                exit(1)

            reason = result.get("reason", "")
            print(f"Good Job! {guessed_word} {start_phrase} {previous_guess}!\n\n{reason}")

            self.memory_storage.set("previous_guess", guessed_word)

if __name__ == "__main__":
    username = input("If you want to play custom game from community, enter user username or pass: ").strip()
